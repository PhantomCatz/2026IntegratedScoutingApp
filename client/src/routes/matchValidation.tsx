...
type AllianceZoneData = {
	[key: TbaApi.MatchKey]:
		| Partial<{
				[allianceColor in TbaApi.AllianceColor]: Database.AllianceZoneEntry;
		  }>
		| undefined;
};
type MatchData = {
...
function MatchValidation(props: Props): React.ReactElement {
		...
		void Promise.all([
			(async () => {
				...
				for (const allianceZoneEntry of data) {
					const id = getMatchId(
						allianceZoneEntry.event_key,
						allianceZoneEntry.comp_level,
						allianceZoneEntry.match_number,
					);
	...
	function getInvalidMatches(): React.ReactElement[] | null {
		...
		const invalidMatches: {
			[matchId: TbaApi.MatchKey]: {
				[alliance in TbaApi.AllianceColor]?: {
		...
		for (const [_id, allianceZoneEntry] of Object.entries(allianceZoneData)) {
			...
			for (const _allianceColor in allianceZoneEntry) {
				...
				if (
					(databaseTotalAutonCount < RATIO_THRESHOLD * tbaTotalAutonCount ||
				...
				if (
					(databaseTotalTeleopCount < RATIO_THRESHOLD * tbaTotalTeleopCount ||
		...
		for (const [_matchId, matchInfo] of Object.entries(invalidMatches)) {
			...
			for (const [_allianceColor, options] of Object.entries(matchInfo)) {
				...
				if (options.needsToScout) {
					invalidItems.push(
						<invalid-match key={`${matchId}|${allianceColor}`}>
							<p>
								Match id:{" "}
								<a href={tbaMatchLink + matchId} target="_blank" rel="noreferrer">
									{matchId}
								</a>
								<br />
					...
					<invalid-match key={`${matchId}|${allianceColor}`}>
						...
						{options.autoMismatch && (
							<invalid-item className="invalid-item__auton-mismatch">
								Calculated Auton Score:{" "}
								{entries
									.map((m) => m.auton_fuel_scored)
									.reduce((a, b) => a + b)
									.toString()}
								<br />
						...
						{options.teleopMismatch && (
							<invalid-item className="invalid-item__teleop-mismatch">
								Calculated Teleop Score:{" "}
								{entries
									.map((m) => m.teleop_fuel_scored)
									.reduce((a, b) => a + b)
									.toString()}
								<br />
	...
	return (
		<>
			...
			<match-validation>
				...
				{invalidMatches ? invalidMatches : <h1>No Data QAQ</h1>}
			</match-validation>
		</>
	);
}
